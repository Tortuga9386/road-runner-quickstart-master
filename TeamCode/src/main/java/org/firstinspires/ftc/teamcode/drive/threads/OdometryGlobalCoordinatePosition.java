...
public class OdometryGlobalCoordinatePosition implements Runnable{
    ...
    public double returnOrientationScaledRadians(){
        return reduce_radians(robotOrientationRadians/4);
    }
    
    public double reduce_radians(double radians) {
        double scaledRadians = Math.atan2(Math.sin(radians), Math.cos(radians));
        // if (scaledRadians<0) {
        //     scaledRadians = Math.abs(scaledRadians) + Math.PI;
        // }
